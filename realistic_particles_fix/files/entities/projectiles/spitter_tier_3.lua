...
for content in nxml.edit_file("data/entities/projectiles/deck/spitter_tier_3.xml", ModTextFileGetContent, ModTextFileSetContent) do
    local explosion = content:first_of("ProjectileComponent"):first_of("config_explosion")
    if explosion then
        explosion:set("sparks_enabled", "1") -- from "0"
        :set("sparks_count_min", "8") -- from unset
        :set("sparks_count_max", "12") -- from unset
        :set("light_enabled", "1")
    ...
    if emitter then
        emitter:set("count_min", "10") -- from "1"
        :set("count_max", "15") -- from "6"
